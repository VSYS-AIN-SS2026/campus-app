-- Extend update_user_profile_info to also allow updating the email field.
-- For authenticated users the email in auth.users is changed via supabase.auth.updateUser()
-- on the client side; this RPC only updates public.users directly (used for the demo user).
DROP FUNCTION IF EXISTS public.update_user_profile_info(text, text);
...
BEGIN
  ...
  SET
    ...
    email       = coalesce(trimmed_email, users.email),
    updated_at  = now()
