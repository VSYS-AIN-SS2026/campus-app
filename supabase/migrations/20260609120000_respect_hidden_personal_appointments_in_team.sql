-- Respektiert den ausgeblendet-Status persönlicher Termine in der Team-Freizeit-
-- Suche und der Team-Wochenansicht.  Ein persönlicher Termin zählt als frei (nicht
-- als Konflikt), wenn sein Besitzer ihn über den Standard-Ausblende-Mechanismus
-- verborgen hat – d. h. wenn ein Eintrag mit series_id = 'personal:<id>' in
-- user_hidden_schedule_series oder occurrence_id = 'personal:<id>' in
-- user_hidden_schedule_occurrences existiert.
...
BEGIN
  ...
  -- Persönliche Termine der Woche (ausgeblendete ausgeschlossen)
  SELECT
    tm.user_id                                                                          AS member_id,
    COALESCE(u.full_name, u.email)                                                      AS member_name,
