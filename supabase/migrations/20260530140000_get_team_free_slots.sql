-- ============================================================
-- Algorithmus: freie Zeitfenster eines Teams für eine Woche
--
-- get_team_free_slots(team, woche, dauer, min_start, max_end,
--                     ausgeschlossene_wochentage, zeitzone)
-- liefert je freiem Zeitfenster (Team komplett frei) start/ende in UTC.
--
-- Konfliktmenge pro Mitglied:
--   * Stundenplan-Slots (user_events) – ALLE; "ausgeblendet" zählt weiter.
--     (Ein "abgewählt"-Konzept existiert backend-seitig noch nicht; sobald
--      vorhanden, hier herausfiltern.)
--   * Termine (team_appointments) mit Einladungsstatus pending oder accepted
--     – über alle Teams des jeweiligen Mitglieds (Mitglied ist dann belegt).
--
-- Zeitzonen: Stundenplan-Zeiten sind lokale Wandzeit je Wochentag und werden
-- mit p_time_zone auf das konkrete Datum der Zielwoche verankert und nach UTC
-- umgerechnet. Termine sind bereits timestamptz (UTC). Ausgabe in UTC.
--
-- Wochentag-Index: 0 = Montag … 6 = Sonntag (wie user_events.day_index).
-- ============================================================
