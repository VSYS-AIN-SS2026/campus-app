-- ============================================================
-- Limit get_team_free_slots appointment conflicts to current team only.
--
-- Previously the RPC considered ALL appointments across ALL teams
-- where a team member had a pending/accepted invitation. Since
-- the calendar only shows appointments for the current team
-- (get_team_appointments filters by p_team_id), these cross-team
-- conflicts were invisible to the user, causing free slots to
-- appear missing on days that looked empty.
--
-- The fix: only query team_appointments WHERE a.team_id = p_team_id,
-- matching what get_team_appointments returns for the calendar.
-- ============================================================
    ...
    FOR rec IN
      ...
      FROM (
        ...
        FROM (
          ...
          -- Wochenansicht keine Terminkonflikte aus anderen Teams zeigt).
