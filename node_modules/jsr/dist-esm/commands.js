...
const pkg_manager_ts_1 = require("./pkg_manager.js");
const download_ts_1 = require("./download.js");
...
async function wrapWithStatus(msg, fn) {
    ...
    try {
        await fn();
        process.stdout.write((0, utils_ts_1.styleText)("green", "ok") + "\n");
    }
    catch (err) {
        process.stdout.write((0, utils_ts_1.styleText)("red", "error") + "\n");
        throw err;
    }
}
async function setupNpmRc(dir) {
    ...
    try {
        ...
        if (!content.includes("@jsr:registry=")) {
            const nl = (0, utils_ts_1.getNewLineChars)(content);
...
async function install(packages, options) {
    const { pkgManager, root } = await (0, pkg_manager_ts_1.getPkgManager)(process.cwd(), options.pkgManagerName);
    if (packages.length > 0) {
        if (pkgManager instanceof pkg_manager_ts_1.Bun && !(await pkgManager.isNpmrcSupported())) {
        ...
        else if (pkgManager instanceof pkg_manager_ts_1.YarnBerry) {
        ...
        console.log(`Installing ${(0, utils_ts_1.styleText)("cyan", packages.join(", "))}...`);
    }
    await pkgManager.install(packages, options);
}
async function remove(packages, options) {
    const { pkgManager } = await (0, pkg_manager_ts_1.getPkgManager)(process.cwd(), options.pkgManagerName);
    console.log(`Removing ${(0, utils_ts_1.styleText)("cyan", packages.join(", "))}...`);
    await pkgManager.remove(packages);
}
async function getOrDownloadBinPath(binFolder, canary) {
    const info = await (0, download_ts_1.getDenoDownloadUrl)(canary);
    ...
    if (!(await (0, utils_ts_1.fileExists)(binPath))) {
        ...
        await (0, download_ts_1.downloadDeno)(binPath, info);
    }
    return binPath;
}
async function publish(cwd, options) {
    ...
    await (0, utils_ts_1.exec)(binPath, args, cwd, env);
}
async function runScript(cwd, script, options) {
    const { pkgManager } = await (0, pkg_manager_ts_1.getPkgManager)(cwd, options.pkgManagerName);
    await pkgManager.runScript(script);
}
async function showPackageInfo(raw) {
    const pkg = utils_ts_1.JsrPackage.from(raw);
    const meta = await (0, api_ts_1.getPackageMeta)(pkg);
    if (pkg.version === null) {
    ...
    const npmInfo = await (0, api_ts_1.getNpmPackageInfo)(pkg);
    const versionInfo = npmInfo.versions[pkg.version];
    const time = npmInfo.time[pkg.version];
    const publishTime = new Date(time).getTime();
    console.log();
    console.log((0, utils_ts_1.styleText)("cyan", `@${pkg.scope}/${pkg.name}@${pkg.version}`) +
        ` | latest: ${(0, utils_ts_1.styleText)("magenta", meta.latest ?? "-")} | versions: ${(0, utils_ts_1.styleText)("magenta", versionCount.toString())}`);
    console.log(npmInfo.description);
    console.log();
    console.log(`npm tarball:   ${(0, utils_ts_1.styleText)("cyan", versionInfo.dist.tarball)}`);
    console.log(`npm integrity: ${(0, utils_ts_1.styleText)("cyan", versionInfo.dist.integrity)}`);
    console.log();
    console.log(`published: ${(0, utils_ts_1.styleText)("magenta", (0, utils_ts_1.timeAgo)(Date.now() - publishTime))}`);
}
