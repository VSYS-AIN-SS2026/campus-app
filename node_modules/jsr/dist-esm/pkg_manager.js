...
exports.Bun = exports.YarnBerry = void 0;
...
async function execWithLog(cmd, args, cwd) {
    console.log((0, utils_ts_1.styleText)("dim", `$ ${cmd} ${args.join(" ")}`));
    return (0, utils_ts_1.exec)(cmd, args, cwd);
}
...
async function isYarnBerry(cwd) {
    ...
    const output = await (0, utils_ts_1.exec)("yarn", ["--version"], cwd, undefined, true);
    const version = output.stdout;
    if (!version) {
        (0, utils_ts_1.logDebug)("Unable to detect yarn version, assuming classic");
        return false;
    }
    if (version.startsWith("1.")) {
        (0, utils_ts_1.logDebug)("Detected yarn classic from version");
        return false;
    }
    (0, utils_ts_1.logDebug)("Detected yarn berry from version");
    return true;
}
class Npm {
    cwd;
...
class YarnBerry extends Yarn {
    ...
    async toPackageArgs(pkgs) {
        ...
        await Promise.all(pkgs.map(async (pkg) => {
            pkg.version ??= `^${await (0, api_ts_1.getLatestPackageVersion)(pkg)}`;
        }));
...
class Bun {
    ...
    async isNpmrcSupported() {
        const output = await (0, utils_ts_1.exec)("bun", ["--version"], this.cwd, undefined, true);
        const version = output.stdout;
...
async function getPkgManager(cwd, pkgManagerName) {
    ...
    const fromEnv = envPkgManager !== undefined
        ...
        : null;
    const { projectDir, pkgManagerName: fromLockfile, root } = await (0, utils_ts_1.findProjectDir)(cwd);
