...
function prettyPrintRow(rows) {
    ...
    return rows
        .map((row) => `  ${(0, utils_ts_1.styleText)("green", row[0].padEnd(max))}  ${row[1]}`)
        .join("\n");
}
function printHelp() {
...
function getPackages(positionals, allowEmpty) {
    ...
    const packages = pkgArgs.map((p) => utils_ts_1.JsrPackage.from(p));
    if (!allowEmpty && pkgArgs.length === 0) {
        console.error((0, utils_ts_1.styleText)("red", `Missing packages argument.`));
        console.log();
        printHelp();
        process.exit(1);
    }
...
if (args.length === 0) {
    ...
    if (cmd === "publish") {
        ...
        run(async () => {
            const projectInfo = await (0, utils_ts_1.findProjectDir)(process.cwd());
            return (0, commands_ts_1.publish)(process.cwd(), {
        ...
        if (pkgName === undefined) {
            console.error((0, utils_ts_1.styleText)("red", `Missing package name.`));
            printHelp();
            process.exit(1);
        }
        run(async () => {
            await (0, commands_ts_1.showPackageInfo)(pkgName);
        });
    }
    else {
        ...
        if (cmd === "i" || cmd === "install" || cmd === "add") {
            run(async () => {
                ...
                await (0, commands_ts_1.install)(packages, {
            ...
            run(async () => {
                ...
                await (0, commands_ts_1.remove)(packages, { pkgManagerName });
            });
        }
        else if (cmd === "run") {
            const script = options.positionals[1];
            if (!script) {
                console.error((0, utils_ts_1.styleText)("red", `Missing script argument.`));
                console.log();
                printHelp();
                process.exit(1);
            }
            run(async () => {
                await (0, commands_ts_1.runScript)(process.cwd(), script, { pkgManagerName });
            });
        }
        else {
            ...
            if (fs.existsSync(packageJsonPath)) {
                ...
                if (packageJson.scripts && packageJson.scripts[cmd]) {
                    run(async () => {
                        await (0, commands_ts_1.runScript)(process.cwd(), cmd, { pkgManagerName });
                    });
                }
                else {
...
async function run(fn) {
    ...
    try {
        ...
        console.log();
        console.log(`${(0, utils_ts_1.styleText)("green", "Completed")} in ${(0, utils_ts_1.prettyTime)(time)}`);
    }
    catch (err) {
        if (err instanceof utils_ts_1.JsrPackageNameError) {
            console.log((0, utils_ts_1.styleText)("red", err.message));
            process.exit(1);
        }
        else if (err instanceof utils_ts_1.ExecError) {
            console.log((0, utils_ts_1.styleText)("red", err.message));
...
function throwUnknownCommand(cmd) {
    console.error((0, utils_ts_1.styleText)("red", `Unknown command: ${cmd}`));
    console.log();
    printHelp();
    process.exit(1);
}
